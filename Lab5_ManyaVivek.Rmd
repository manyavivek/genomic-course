...
time_series_long_deaths_US <- time_series_deaths_US %>% 
                 pivot_longer(-c(Province_State, Country_Region, UID, iso2, iso3, code3, FIPS, Lat, Long_, Combined_Key, Admin2, Population),
                            names_to = "Date", values_to = "Deaths") %>% 
    unite(Key, Province_State, Date, Deaths, sep = ".", remove = FALSE)
...
```{r, fig.height=20, fig.width = 10}
time_series_joined_US_mutate %>% 
  ggplot(aes(x = Date.x, y = Confirmed), size = 0.5) +
  geom_point() + 
  facet_wrap(~Province_State.y, ncol = 5) + 
  labs(x = "Date", 
       y = "Confirmed",
