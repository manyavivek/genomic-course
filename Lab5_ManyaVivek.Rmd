...
## Exercise 9 
```{r}
time_series_long_confirmed_US <- time_series_long_confirmed_US %>% 
  count(Province_State, Date, Confirmed) %>% 
  filter(!is.na(Province_State))
```

```{r, fig.height= 25, fig.width = 10}
    ggplot(data = time_series_long_confirmed_US, mapping = aes(x = Date, y = Confirmed)) +
    geom_point() +
    geom_line() +
